...
def main(args):
    ...
    best_acc1 = 0.0  # 初始化最佳准确率
    best_checkpoint_path = None  # 初始化最佳模型路径
    for epoch in range(args.start_epoch, args.epochs):
        ...
        """Save best acc1 checkpoint"""
        acc1_value = test_stats['acc1']
        if acc1_value > best_acc1:
            best_acc1 = acc1_value
            best_epoch = epoch + 1

            if args.output_dir:
                # if best model exist, remove it
                if best_checkpoint_path is not None and os.path.exists(best_checkpoint_path):
                    os.remove(best_checkpoint_path)
                # save the best model，named "best_checkpoint_<epoch>.pth"
                best_checkpoint_path = output_dir / args.dataset_name / args.output_sub_dir / f'best_checkpoint_{best_epoch}.pth'
                utils.save_on_master({
                    'model': model_without_ddp.state_dict(),
                    'optimizer': optimizer.state_dict(),
                    'lr_scheduler': lr_scheduler.state_dict(),
                ...
                print(f'Saved best model: {best_checkpoint_path} with acc1: {best_acc1:.4f} at epoch: {best_epoch}')
        #####################################################################
