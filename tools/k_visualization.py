...
from k_engine import train_one_epoch
...
def get_args_parser():
    ...
    parser.add_argument('--test', default="val", type=str, choices=["test", "val"])
...
def SuperImposeHeatmap(attention, input_image):
    alpha = 0.5
    avg_heatmap_resized = cv2.resize(attention, (input_image.shape[1], input_image.shape[0]),
                                     interpolation=cv2.INTER_CUBIC)
    avg_normalized_heatmap = (avg_heatmap_resized - np.min(avg_heatmap_resized)) / (
                np.max(avg_heatmap_resized) - np.min(avg_heatmap_resized))
...
def visualize_heads(encoder_output, attention_score_gt_query, avg_attention_score_gt_query, prediction, img_id,
                    image_file, des_dir):
    ...
    for head_index in range(attention_score_gt_query.shape[1]):
        ...
        heatmap_head = attention_score_gt_query[:, head_index, :].reshape(encoder_output.shape[2], encoder_output.shape[
            3]).detach().cpu().numpy()
        ...
        if head_index == 7:
            avg_heatmap_head = avg_attention_score_gt_query.reshape(encoder_output.shape[2],
                                                                    encoder_output.shape[3]).detach().cpu().numpy()
...
def visualize_queries(encoder_output, attention_score, avg_attention_score, prediction, img_id, image_file, des_dir,
                      similar_queries):
    ...
    for query_index in (similar_queries):
        ...
        avg_heatmap = avg_attention_score_query.reshape(encoder_output.shape[2],
                                                        encoder_output.shape[3]).detach().cpu().numpy()
    ...
    for head_index in range(attention_score.shape[1]):
        ...
        for query_index in (similar_queries):
            heatmap_query = attention_score_head[:, query_index, :].reshape(encoder_output.shape[2],
...
def visualization(args, filename, encoder_output, attention_scores, avg_attention_scores, similar_queries, image_label,
                  prediction):
    """
    ...
    if args.gt_query_heads == 1:
        ...
        visualize_heads(encoder_output, attention_score_gt_query, avg_attention_score_gt_query, prediction, img_id,
                        image_file, des_dir)
    ...
    if args.sim_query_heads == 1:
        ...
        image_file = get_image_filename(args, filename)
        visualize_queries(encoder_output, attention_score, avg_attention_score, prediction, img_id, image_file, des_dir,
                          similar_queries)
...
def evaluate(args, model, data_loader, device):
    ...
    for samples, targets in metric_logger.log_every(data_loader, 100, header):
        ...
        if image_label == args.class_index:
            ...
            outputs, query, encoder_output, _, attention_scores, avg_attention_scores = model(samples)
            ...
            visualization(args, filename, encoder_output, attention_scores, avg_attention_scores, similar_queries,
