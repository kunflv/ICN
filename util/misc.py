...
def load_model(args, state_dict):
    ...
    """Delete existing INTR heads keys"""
    # keys_to_delete = ["presence_vector.weight", "presence_vector.bias"]
    #
    # for key in keys_to_delete:
    #     del state_dict[key]

    """New classifier head (KAN) for image level prediction for ICN"""
    grid_range = [-1, 1]
    spline_order = 3
    grid_size = 5  # 网格大小，默认为 5
    h = (grid_range[1] - grid_range[0]) / grid_size  # 计算网格步长

    state_dict['kan_cls.layers.0.base_weight'] = nn.init.xavier_uniform_(torch.empty(1, args.hidden_dim))  # 初始化基础权重和分段多项式权重
    state_dict['kan_cls.layers.0.spline_weight'] = nn.init.xavier_uniform_(torch.empty(1, args.hidden_dim,
                                                            grid_size + spline_order))
    state_dict['kan_cls.layers.0.spline_scaler'] = nn.init.xavier_uniform_(
                                                           torch.empty(1, args.hidden_dim))

    state_dict['kan_cls.layers.0.grid'] = ( # 生成网格
            (torch.arange(-spline_order, grid_size + spline_order + 1) * h
                + grid_range[0]).expand(args.hidden_dim, -1).contiguous())
